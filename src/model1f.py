...
class BERT1F(nn.Module):
    def __init__(self, config):
        super(BERT1F, self).__init__()
    ...
    def forward(self, features, s_span_set, t_span_set, labels):
        ...
        maps = torch.zeros((len(features), 1, self.max_seq_len, self.max_seq_len),
                           requires_grad=True).cuda()  # 1dim for channel
        for i in range(len(features)):
            s = torch.cat((sent_emb[i, 0:len(features[i].input_ids_a), :],
                           torch.zeros((self.max_seq_len - len(features[i].input_ids_a), self.bert_emb_dim)).cuda()))
            t = torch.cat((sent_emb[i,
                           len(features[i].input_ids_a):len(features[i].input_ids_a) + len(features[i].input_ids_b), :],
                           torch.zeros((self.max_seq_len - len(features[i].input_ids_b), self.bert_emb_dim)).cuda()))
